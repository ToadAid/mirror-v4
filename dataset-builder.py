...
CANONICAL_SYSTEM_PROMPT = (
    ...
    "You answer with symbolic depth, thematic resonance, and poignant guiding questions. "
...
class JSONLBuilder(QWidget):
    ...
    def toggle_system_lock(self, state):
        if state == 2:
            self.system_input.setPlainText(CANONICAL_SYSTEM_PROMPT)
    ...
    def _call_llm(self, system_prompt: str, user_prompt: str) -> str:
        fmt = (
            ...
            "2. Write 4-8 lines of poetic, profound response\n"
            "3. Include a \"**Guiding Question:**\" line with a poignant question\n"
            "4. End with 2-4 symbols from: 🪞🌊🍃🌀\n"
            "5. English only\n"
            "6. Timeless, precise voice\n"
        )
        body = {
            "messages": [
                {"role": "system", "content": (system_prompt or CANONICAL_SYSTEM_PROMPT) + fmt},
    ...
    def generate_gpt_reply(self):
        system_prompt = self.system_input.toPlainText().strip() or CANONICAL_SYSTEM_PROMPT
    ...
    def normalize_text(self, text: str) -> str:
        ...
        # Ensure "Traveler," at start (case-insensitive)
        if not re.match(r"^\s*Traveler,\s*$", t.splitlines()[0], flags=re.IGNORECASE):
            if not t.lower().startswith("traveler,"):
                t = "Traveler,\n\n" + t

        # Ensure Guiding Question line exists
        if "**Guiding Question:**" not in t:
            t = t.rstrip() + "\n\n**Guiding Question:** What harmony do you seek to restore within yourself?\n"

        # Ensure 2–4 symbols at end
        sym_re = r"[{}]\s*[{}]?\s*[{}]?\s*[{}]?$".format("".join(SYMBOLS), "".join(SYMBOLS),
    ...
    def generate_batch(self):
        ...
        base_system = self.system_input.toPlainText().strip() or CANONICAL_SYSTEM_PROMPT
        ...
        for i, q in enumerate(questions, 1):
            ...
            try:
                ans = self._call_mirror(q) if engine.startswith("Mirror") else self._call_llm(base_system if sys_per_item else CANONICAL_SYSTEM_PROMPT, q)
        ...
        try:
            ...
            if path.lower().endswith(".csv"):
                with open(path, "r", encoding="utf-8") as f:
                    try: peek = f.read(2048); f.seek(0); dialect = csv.Sniffer().sniff(peek)
            ...
            if not lines: QMessageBox.information(self,"No Questions Found","File contained no usable lines."); return
    ...
    def save_jsonl(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Save JSONL File", "", "JSONL Files (*.jsonl)")
        if path:
            with open(path, "w", encoding="utf-8") as f:
                for entry in self.dataset: f.write(json.dumps(entry, ensure_ascii=False) + "\n")
    ...
    def on_table_selection_changed(self): self.preview_box.setPlainText(self.current_row_jsonl())
    ...
    def copy_preview(self): QApplication.clipboard().setText(self.preview_box.toPlainText(), QClipboard.Clipboard)
    ...
    def refresh_stats(self):
        ...
        avg_user = (user_chars / n) if n else 0
        avg_asst = (asst_chars / n) if n else 0
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("🪞 Mirror Forge v13 — Compact + Normalize + Coverage")
        ...
        act_export_csv = QAction("Export Table as CSV", self, triggered=self.export_csv)
        ...
        tools_menu.addActions([act_gen, act_cancel, act_clear, act_clone, act_template, act_delete, act_normalize])
        ...
        help_menu.addAction(QAction("About", self, triggered=lambda: QMessageBox.information(
            self,"Mirror Forge",
            "Compact Mode (View) shrinks editors.\n"
            "Normalize Selected (Tools) enforces Traveler/GQ/symbol seal.\n"
            "Delete Selected (Tools) removes chosen rows.\n"
        )))
        ...
        self.lock_system_checkbox = QCheckBox("Lock to Canonical"); self.lock_system_checkbox.setChecked(True)
        ...
        self.user_input.setPlaceholderText("User message…")

        self.assistant_input = QTextEdit()
        self.assistant_input.setPlaceholderText("Assistant message…")

        # Force light theme boxes for User/Assistant (black text on white box)
        def force_light_qtextedit(w: QTextEdit):
            pal = w.palette()
            pal.setColor(QPalette.Base, Qt.white)   # viewport background
            pal.setColor(QPalette.Text, Qt.black)   # text color
            w.setPalette(pal)
        ...
        # User row with Clear button
        ur = QHBoxLayout()
        ur.addWidget(QLabel("User"))
        self.clear_user_btn = QPushButton("Clear")
        self.clear_user_btn.setToolTip("Clear user input")
        self.clear_user_btn.clicked.connect(self.user_input.clear)
        ur.addWidget(self.clear_user_btn)
        ur.addStretch(1)
        root.addLayout(ur)
        root.addWidget(self.user_input)

        # Assistant row with Generate button
        ar = QHBoxLayout()
        ar.addWidget(QLabel("Assistant"))
        gen_btn = QPushButton("Generate", clicked=self.generate_gpt_reply)
        ar.addWidget(gen_btn)
        ar.addStretch(1)
        root.addLayout(ar)
        root.addWidget(self.assistant_input)

        # Hook up live token count
        ...
        er.addWidget(QLabel("🔧 LLM URL:")); self.gpt_url_input = QLineEdit(self.api_url); self.gpt_url_input.textChanged.connect(self.update_gpt_url); er.addWidget(self.gpt_url_input)
        er.addWidget(QLabel("🪞 Mirror URL:")); self.mirror_url_input = QLineEdit(self.mirror_url); self.mirror_url_input.textChanged.connect(self.update_mirror_url); er.addWidget(self.mirror_url_input)
